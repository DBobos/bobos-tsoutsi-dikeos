...
df_hmu_mtway['cumulative_distance'] = df_hmu_mtway.groupby(['contestant'])['distance'].cumsum(axis=0)
...
df_hmu_urblt = positioning(df_hmu_urblt)
...
df_hmu_urblt = lane_off(df_hmu_urblt)
...
df_ntu_mtway = positioning(df_ntu_mtway)
...
df_ntu_mtway = lane_off(df_ntu_mtway)
...
df_ntu_urblt = positioning(df_ntu_urblt)
...
# calculate the lane offset in meters as the distance of each contestant's
# position from the route coordinates, for df_ntu_mtway
df_ntu_urblt = lane_off(df_ntu_urblt)
...
""" HMU MOTORWAY """
# brake, headway_s, steering, lane_offset_m

# normalize brake to 0-1
df_hmu_mtway['brake'] = MinMaxScaler().fit_transform(np.array(df_hmu_mtway['brake']).reshape(-1,1))
# normalize headway to 0-1
df_hmu_mtway['headway_s'] = MinMaxScaler().fit_transform(np.array(df_hmu_mtway['headway_s']).reshape(-1,1))
# normalize steering to -1 - 1
df_hmu_mtway['steering'] = df_hmu_mtway['steering'].abs()
df_hmu_mtway['steering']= df_hmu_mtway['steering'] /df_hmu_mtway['steering'].abs().max()
# lane_offset normalize
df_hmu_mtway['lane_offset_m'] = MinMaxScaler().fit_transform(np.array(df_hmu_mtway['lane_offset_m']).reshape(-1,1))

# export df to csv
df_hmu_mtway.to_csv('df_hmu_mtway.csv', index=True)
...
# normalize headway to 0-1
df_hmu_urblt['headway_s'] = MinMaxScaler().fit_transform(np.array(df_hmu_urblt['headway_s']).reshape(-1,1))
# normalize steering to -1 - 1
df_hmu_mtway['steering'] = df_hmu_mtway['steering'].abs()
df_hmu_urblt['steering']= df_hmu_urblt['steering'] /df_hmu_urblt['steering'].abs().max()
# lane_offset normalize to -1 - 1
df_hmu_urblt['lane_offset_m']= df_hmu_urblt['lane_offset_m'] /df_hmu_urblt['lane_offset_m'].abs().max()

# export df to csv
df_hmu_urblt.to_csv('df_hmu_urblt.csv', index=True)
...
# normalize brake to 0-1
df_ntu_mtway['brake'] = MinMaxScaler().fit_transform(np.array(df_ntu_mtway['brake']).reshape(-1,1))
# normalize headway to 0-1
df_ntu_mtway['headway_s'] = MinMaxScaler().fit_transform(np.array(df_ntu_mtway['headway_s']).reshape(-1,1))
# normalize steering to -1 - 1
df_ntu_mtway['steering'] = df_ntu_mtway['steering'].abs()
df_ntu_mtway['steering']= df_ntu_mtway['steering'] /df_ntu_mtway['steering'].abs().max()
# lane_offset normalize to -1 - 1
df_ntu_mtway['lane_offset_m']= df_ntu_mtway['lane_offset_m'] /df_ntu_mtway['lane_offset_m'].abs().max()

# export df to csv
df_ntu_mtway.to_csv('df_ntu_mtway.csv', index=True)



""" NTU URBAN """
# normalize brake to 0-1
df_ntu_urblt['brake'] = MinMaxScaler().fit_transform(np.array(df_ntu_urblt['brake']).reshape(-1,1))
# normalize headway to 0-1
df_ntu_urblt['headway_s'] = MinMaxScaler().fit_transform(np.array(df_ntu_urblt['headway_s']).reshape(-1,1))
# normalize steering to -1 - 1
df_ntu_urblt['steering'] = df_ntu_urblt['steering'].abs()
df_ntu_urblt['steering']= df_ntu_urblt['steering'] /df_ntu_urblt['steering'].abs().max()
# lane_offset normalize to -1 - 1
df_ntu_urblt['lane_offset_m']= df_ntu_urblt['lane_offset_m'] /df_ntu_urblt['lane_offset_m'].abs().max()


# export df to csv
df_ntu_urblt.to_csv('df_ntu_urblt.csv', index=True)
...
# mtway_columns are missing orientation column which is present in urblt_columns
# so, we use mtway_columns list for the creation of the final dataframe
final_dataframes = [df_m[[c for c in mtway_columns]],
                    df_u[[c for c in mtway_columns]]
                    ]

# create the final dataframe (df_m) for both scenarios
df = pd.concat(final_dataframes, ignore_index=True)

# export df to csv
df.to_csv('timeseries_data.csv', index=True)
